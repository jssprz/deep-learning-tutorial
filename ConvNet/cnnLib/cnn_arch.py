...
def vgg16_net_fn(features, input_shape, n_classes, n_channels, is_training=True):
    with tf.variable_scope("net_scope"):
        ...
        # 64-channels block
        conv_1 = layers.conv_layer(x_tensor, shape=[3, 3, n_channels, 64], stride=1, name='conv3-64',
                                   is_training=is_training)
        print(" conv_1: {} ".format(conv_1.get_shape().as_list()))

        conv_2 = layers.conv_layer(conv_1, shape=[3, 3, 64, 64], stride=1, name='conv3-64',
                                   is_training=is_training)
        print(" conv_2: {} ".format(conv_2.get_shape().as_list()))
        ...
        # 128-channels block
        conv_3 = layers.conv_layer(pool_1, shape=[3, 3, 64, 128], stride=1, name='conv3-128',
                                   is_training=is_training)
        print(" conv_3: {} ".format(conv_3.get_shape().as_list()))

        conv_4 = layers.conv_layer(conv_3, shape=[3, 3, 128, 128], stride=1, name='conv3-128',
                                   is_training=is_training)
        ...
        # 256-channels block
        conv_5 = layers.conv_layer(pool_2, shape=[3, 3, 128, 256], stride=1, name='conv3-256',
                                   is_training=is_training)
        print(" conv_5: {} ".format(conv_5.get_shape().as_list()))

        conv_6 = layers.conv_layer(conv_5, shape=[3, 3, 256, 256], stride=1, name='conv3-256',
                                   is_training=is_training)
        print(" conv_6: {} ".format(conv_6.get_shape().as_list()))

        conv_7 = layers.conv_layer(conv_6, shape=[3, 3, 256, 256], stride=1, name='conv3-256',
                                   is_training=is_training)
        ...
        # 512-channels block1
        conv_8 = layers.conv_layer(pool_3, shape=[3, 3, 256, 512], stride=1, name='conv3-512',
                                   is_training=is_training)
        print(" conv_8: {} ".format(conv_8.get_shape().as_list()))

        conv_9 = layers.conv_layer(conv_8, shape=[3, 3, 512, 512], stride=1, name='conv3-512',
                                   is_training=is_training)
        print(" conv_9: {} ".format(conv_9.get_shape().as_list()))

        conv_10 = layers.conv_layer(conv_9, shape=[3, 3, 512, 512], stride=1, name='conv3-512',
                                    is_training=is_training)
        ...
        # 512-channels block2
        conv_11 = layers.conv_layer(pool_4, shape=[3, 3, 512, 512], stride=1, name='conv3-512',
                                    is_training=is_training)
        print(" conv_11: {} ".format(conv_11.get_shape().as_list()))

        conv_12 = layers.conv_layer(conv_11, shape=[3, 3, 512, 512], stride=1, name='conv3-512',
                                    is_training=is_training)
        print(" conv_12: {} ".format(conv_12.get_shape().as_list()))

        conv_13 = layers.conv_layer(conv_12, shape=[3, 3, 512, 512], stride=1, name='conv3-512',
                                    is_training=is_training)
        ...
        fc1 = layers.fc_layer(pool_5, 4096, name='fc-4096')
        print(" fc1: {} ".format(fc1.get_shape().as_list()))
        fc2 = layers.fc_layer(fc1, 4096, name='fc-4096')
        print(" fc2: {} ".format(fc2.get_shape().as_list()))

    with tf.variable_scope("class_layer"):
        fc3 = layers.fc_layer(fc2, n_classes, name='fc-{}'.format(n_channels), use_relu=False)
        print(" fc3: {} ".format(fc3.get_shape().as_list()))
